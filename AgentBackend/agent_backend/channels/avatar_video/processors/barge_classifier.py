...
during an avatar-video call.

ISOLATION NOTE
--------------
Self-contained copy for the avatar_video channel. Imports nothing from
`channels.voice` / `channels.pipecat`. Editing avatar barge behavior here
never touches the voice channel.

Five intents (identical semantics to the voice channel — this is the proven
production decision tree):

    ACK        — listener noise / acknowledgement (yes, mhm, ah, okay).
                 Avatar keeps speaking. Transcript recorded so the brain
                 knows the user is engaged.
    ANSWER     — short ack that answers a question the avatar just asked.
                 Avatar keeps speaking (don't cancel mid-question); transcript
    ...
    AMBIGUOUS  — heuristic can't decide. Manager keeps the avatar speaking and
                 re-classifies on the FULL final transcript. No async LLM on
                 the critical path — latency first.

Three architectural rules:

  1. **No latency on the critical path.** The heuristic is <5 ms and decides
     90%+ of cases. There is NO LLM call here at all — the manager waits for
     the full final transcript on AMBIGUOUS instead of guessing on fragments.

  2. **No trial-and-error vocab tuning.** Decision tree over signals we already
     have: STT transcript, conversation history, the avatar's last sentence
     (ends with '?'), and acoustic features.

  3. **Context aware.** A user answering "yes" to a yes/no question is ANSWER,
     not ACK. A single "what?" is CONFUSED; "what about fees" is a question →
     INTERRUPT.

AVATAR-SPECIFIC ACOUSTIC GATE
-----------------------------
The voice channel fights phone-speaker→mic echo with an RMS-ratio check on
raw input audio. In the avatar pipeline the browser does WebRTC AEC AND the
raw InputAudioRawFrames are dropped by InputAudioSink right after STT — so by
the time the manager runs there is no per-frame input RMS to read. The gate
therefore degrades gracefully: when we have no reliable RMS signal it relies
on duration + the manager's bot-speaking grace window + the transcript
heuristic instead. When RMS IS available (caller passes it) the echo ratio
check still applies.
"""
...
# avatar's last utterance ended with '?'.
...
class BargeClassifier:
    ...
    #
    # AVATAR ADAPTATION: input RMS is often unavailable (raw audio dropped by
    # InputAudioSink + browser AEC). When `user_rms` / `tts_peak_rms` are 0
    # we SKIP the echo-ratio check and rely on duration only — the transcript
    # phase + the manager's bot-speaking grace window catch real intent.
    # ------------------------------------------------------------------
    def acoustic_gate_passed(
        ...
        *,
        ...
        if user_duration_ms < 300:
            return False
        # Echo check — ONLY when we actually have RMS signal on both sides.
        # In the avatar pipeline these are usually 0 (no raw input frames),
        # so this branch is skipped and we lean on the transcript heuristic.
    ...
    def _bot_asked_question(self) -> bool:
        """True only if the avatar's last utterance ENDS with a question mark."""
