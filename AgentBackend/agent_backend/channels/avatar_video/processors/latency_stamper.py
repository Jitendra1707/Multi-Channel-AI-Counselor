...
Insert two instances (composer wires them):
  INBOUND  side: stamps stt_first_interim, stt_final — after STT
  OUTBOUND side: stamps brain_first_token, brain_total, tts_first_audio —
                 between AgentBridge and TTS
...
frame. It is gated OFF by default in production (AVATAR_ENABLE_METRICS) — flip
...
from agent_backend.channels.avatar_video.events import EventBus, LatencyEvent
...
class LatencyStamper(FrameProcessor):
    """One processor handles inbound + outbound timing for one session."""
