...
avatar_video detector). Imports only its sibling `events` module and config —
...
In a multi-party room the events are room-level ("SOMEONE is speaking"), which
is exactly what the SilenceMonitor needs — silence only counts when NOBODY
(agent included) is talking.

Public surface
--------------
- Insert this processor BETWEEN the STT stage (after InputAudioSink) and the
...
from agent_backend.channels.meeting.events import EventBus, TurnEvent, TurnState
...
class TurnDetector(FrameProcessor):
    """Per-meeting FSM over multi-source signals."""

    def __init__(self, *, bus: EventBus) -> None:
        super().__init__()
        self._bus = bus
        s = get_settings()
        # Meeting-prefixed tunables so they're independent of the other channels.
        self._brief_pause_s = s.meeting_turn_brief_pause_ms / 1000.0
        self._complete_s    = s.meeting_turn_complete_ms / 1000.0
        self._abandoned_s   = s.meeting_turn_abandoned_s
        self._conf_floor    = s.meeting_turn_confidence_floor
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection) -> None:
        ...
        if isinstance(frame, StartFrame) and self._watchdog_task is None:
            self._watchdog_task = asyncio.create_task(
                self._watchdog(), name="meeting-turn-watchdog",
            )
    ...
    async def _watchdog(self) -> None:
        try:
            ...
            log.warning("[meeting-turn] watchdog crashed: %s", e)
    ...
    def _publish(self, state: TurnState, *, confidence: float, source: str, now: float) -> None:
        ...
        if state != self._last_state:
            log.debug("[meeting-turn] %s -> %s (conf=%.2f src=%s)", self._last_state, state, confidence, source)
