...
class AgentSoulXVideoService(FrameProcessor):
    ...
    def __init__(
        ...
        *,
        ...
        # UNIQUE per-instance SoulX session id. Previously every connection used
        # the hardcoded "pipecat" id — so two concurrent sessions (a real meeting
        # + an avatar_video call, or a stale session that didn't close + a new
        # one) COLLIDED on the same GPU session, contending for the same render
        # state → frame delivery collapsed (~6 fps, ratio 0.25, 20-30s lag,
        # broken audio). A unique id isolates each session on the GPU. A fresh
        # uuid per instance; the runner may pass a stable one (e.g. the room id).
        import uuid

        self._session_id = (session_id or "").strip() or f"pipecat-{uuid.uuid4().hex[:12]}"
        ...
        # AND bursty frame delivery — these two catch exactly that (the "voice
        # breaks mid-sentence" symptom). Reset per utterance. Ported from the
        # seamless avatar_video SoulX so the meeting has the same diagnostics.
        ...
        # Audio-push BURST probe (observe-only). The browser audio jitter buffer
        # balloons — and then GLITCHES on the next underrun — when audio is pushed
        # faster than real-time (a video burst → many 40ms chunks shoved in at
        # once). Over a sliding 500ms wall window, track the peak (audio-ms-pushed
        # / wall-ms) ratio and the largest 200ms-window burst.
        #   peak_audio_push_rate >> 1.0 ⇒ bursty push IS the glitch source. ≈1.0 ⇒ it isn't.
    ...
    def _record_audio_push(self, chunk: bytes) -> None:
        """Burst probe (observe-only): update the sliding-window peak audio-push
        rate. Each `chunk` is int16 16kHz mono, so audio-ms = samples / 16. A peak
        (audio-ms / wall-ms) over 500ms that is >> 1.0 means we're shoving audio in
        faster than real-time — the browser audio-buffer inflation that then
        glitches on the following underrun."""
        now = time.monotonic()
    ...
    async def _connect(self) -> None:
        ...
        init: dict = {"session_id": self._session_id}
        if self._image_path:
    ...
    async def _ensure_connected(self) -> bool:
        """Reconnect-on-demand. The SoulX WS runs to a REMOTE GPU server and can
        drop mid-meeting (idle timeout / network blip / GPU restart) — with no
        reconnect every later turn was silent (ws_ok=False forever). Call this at
        the START of a turn: if the socket is down, tear down stale state and
        re-open it so the agent's voice returns. Returns True if usable."""
        if self._ws is not None and not self._ws.closed:
            return True
        logger.warning("[soulx] WS down at turn start — reconnecting…")
        # Reset per-utterance flags + cancel the dead video consumer before reopen.
        try:
            await self._disconnect()
        except Exception as exc:  # noqa: BLE001
            logger.debug("[soulx] pre-reconnect cleanup failed: {}", exc)
        self._speak_start_sent = False
        self._thinking_started = False
        self._speaking = False
        try:
            await self._connect()
            # The render gate (client-connected) was already released earlier in
            # the meeting; re-arm it so the fresh socket can speak immediately.
            try:
                self.signal_client_connected()
            except Exception:  # noqa: BLE001
                pass
            logger.info("[soulx] reconnect OK")
            return True
        except Exception as exc:  # noqa: BLE001
            logger.error("[soulx] reconnect FAILED: {}", exc)
            return False
                ...
                if msg.type == aiohttp.WSMsgType.BINARY:
                    ...
                    if msg_type_byte == 1:
                        ...
                        if self._ignore_binary:
                            ...
                            continue
                        # A/V probe: time SoulX's delivery cadence at ARRIVAL
                        # (before decode), so a bursty stream (clumps + gaps that
                        # still average 25fps) is visible — that burstiness is the
                        # likely mid-sentence audio glitch.
                    ...
                    if data.get("type") == "speaking":
                        ...
                        if self._speaking:
                            ...
                            _t0 = self._t_speak_start  # capture before reset (A/V offset metrics)
                            ...
                            if (
                                ...
                                # A/V offset: first-audio vs first-frame (≈0 = pairing
                                # holds; large = audio reaches its track ahead of video)
                                # and inter-frame arrival jitter (large gaps = bursty
                                # SoulX delivery → the mid-sentence audio break).
                            ...
                            logger.debug("[soulx] utterance complete — idle resumed")
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection) -> None:
        ...
        if isinstance(frame, StartFrame):
            ...
            self._llm_response_done = False
            # RECONNECT-ON-DEMAND: the remote SoulX GPU WS can drop mid-meeting;
            # without this, ws_ok stays False and the agent is silent for the rest
            # of the call. Re-open it now so this turn's voice/video can flow.
            await self._ensure_connected()
            # Switch server to thinking frames while the LLM processes.
            logger.debug("[soulx] LLMFullResponseStartFrame received ws_ok={} speak_start_sent={}",
            ...
            if self._ws and not self._ws.closed and not self._speak_start_sent:
                try:
                    ...
                    logger.debug("[soulx] thinking message sent")
                except Exception as exc:
            ...
            if self._speak_start_sent:
                ...
                if self._ws and not self._ws.closed:
                    try:
                        ...
                        # Switch the avatar to THINKING immediately so it never
                        # freezes on the last speaking frame while the new turn is
                        # prepared. The server (preemptible) aborts the in-flight
                        # batch within ~1 frame and returns to idle, then this
                        # flips it straight to thinking frames — so a barge-in
                        # reads as instant rather than a multi-hundred-ms freeze.
                        # (The new turn's LLMFullResponseStart re-sends "thinking"
                        # harmlessly.) Ported from the seamless avatar_video SoulX.
                        await self._ws.send_str(json.dumps({"type": "thinking"}))
                        self._thinking_started = True
                    except Exception:
                        pass
                ...
                self._speaking         = False
                self._speak_start_sent = False
                # Reset per-utterance pacing/probe counters so the barged
                # utterance doesn't bleed into the next [soulx-pacing] line.
                self._first_speaking_frame_logged = False
                self._pace_frames = 0
                self._pace_audio_bytes = 0
                self._pace_first_frame_t = None
